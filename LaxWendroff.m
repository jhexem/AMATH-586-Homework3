...
for k = 2:length(tvals)
    for j = 2:(length(xvals)-1)
        U(j, k) = U(j, k-1) - (dt / (2 * dx)) * (U(j+1, k-1) - U(j-1, k-1)) + ...
            (dt^2 / (2 * dx^2)) * (U(j+1, k-1) - 2 * U(j, k-1) + U(j-1, k-1));
    end
...
title("Lax-Wendroff Scheme Solution at t=0.8")
legend({"True Solution", "Lax-Wendroff Scheme Solution"}, 'Location', 'northwest');
xlabel("x")
ylabel("u(0.8, x)")