...
for k = 2:length(tvals)
    ...
    for j = 3:(length(xvals)-1)
        ...
        F1 = U(j, k-1) + (1/2) * (1-(dt/dx)) * (U(j+1, k-1) - U(j, k-1)) * phi(theta1);
        F2 = U(j-1, k-1) + (1/2) * (1-(dt/dx)) * (U(j, k-1) - U(j-1, k-1)) * phi(theta2);
...
title("High Resolution Scheme with Minmod Flux Limiter Solution at t=0.8")
